...
#' @param rounding Whether the coarsening interval is rounded up or down
...
#' @param duration Duration of enrollment in days
#' @param followup Duration of follow-up from end of enrollment in days
#' @param width Censoring interval width
#' @param rounding Whether the coarsening interval is rounded up or down
#' @return A list containing the true survival time and 
#' generated data frames with interval-censored times
generate_df <- function(n, dist, pars, duration, followup, width, rounding) {
  times <- generate_times(n, dist, pars)
  obs.times <- if(rounding == "Rounded up") ceiling(times) else floor(times)
  df.rcensor <- generate_right_censoring(obs.times, duration, followup)
  alive <- df.rcensor$status == "Alive"
  df.intervals <- cbind(df.rcensor, 
                        generate_intervals(df.rcensor$exit, alive, width, rounding))
...
repeat_estimation <- function(iter, days, df.pars, dist.pars) {
  ...
  for(i in 1:iter) {
    ...
    est <- get_estimates(days, df.lst[[2]], df.pars[["dist"]])
    est.prob[i,,] <- est[[1]]
    est.pars[i,,] <- est[[2]]
  }
...
average_bias <- function(thetas, true.vals) {
  bias <- apply(thetas, c(1, 3), function(v) v - true.vals)
  avg.bias <- apply(bias, c(1, 3), mean)
  return(avg.bias)
}
